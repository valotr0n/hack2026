...
async def check_flashcard(req: QuizCheckRequest) -> QuizCheckResponse:
    system = (
        "Ты — справедливый преподаватель. "
        "Оценивай ответ студента по смыслу, а не дословному совпадению. "
        "Если студент передал главную суть — это правильный ответ. "
        "Снижай оценку только если упущена действительно важная часть смысла. "
        "Не штрафуй за отсутствие второстепенных деталей. "
        "Отвечай строго в формате JSON без лишнего текста."
    )
    user = (
        f"Вопрос: {req.question}\n"
        f"Эталонный ответ: {req.correct_answer}\n"
        f"Ответ студента: {req.user_answer}\n\n"
        "Оцени правильность по смыслу. is_correct=true если суть передана верно (score >= 0.6).\n"
        "Верни JSON:\n"
        '{"is_correct": true/false, "score": 0.0-1.0, '
        '"feedback": "краткое объяснение — похвали если верно, укажи что упущено если неверно"}'
    )
